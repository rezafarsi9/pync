class VideoCarousel extends HTMLElement {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                ...
                :host {
                    ...
                    // background: linear-gradient(to right,rgb(31, 10, 76),rgb(41, 0, 145)); /* Tundora grey to black */
            ...
            <div class="carousel-container">
                <div class="carousel-title">ببین چجوری کار می کنه</div>
                <div class="carousel-track" id="carouselTrack">
                    ${this._videos.map((video, index) => `
                        <div class="video-card ${index === 0 ? 'active' : index === 1 ? 'next' : index === 2 ? 'far-next' : 'hidden'}" data-index="${index}">
                            ...
                            <video class="video-element" loop muted preload="metadata" playsinline>
                            ...
                            <div class="video-controls">
                                <div class="control-btn mute-unmute-btn" data-action="mute-unmute">
                                ...
                                <div data-product-name="سرم درخشان کننده لومینوس" data-product-price="12.00" data-product-image="/assets/img/luminus1.jpg">
                                    <div class="control-btn" data-action="addtocart">
    ...
    setupEventListeners() {
        ...
        this.shadowRoot.addEventListener('click', (e) => {
            ...
            switch (action) {
                case 'play-pause':
                    e.stopPropagation();
                    this.togglePlayPause(e.target.closest('.video-card'));
                    break;
                case 'mute-unmute':
                    e.stopPropagation();
                    this.toggleMute(e.target.closest('.video-card'));
                    break;
                case 'like':
    ...
    updateCarousel() {
        ...
        this.cards.forEach((card, index) => {
            ...
            // Update icon states for the new active video
            const video = card.querySelector('.video-element');
            const playPauseBtn = card.querySelector('.play-pause-btn[data-action="play-pause"]');
            const muteBtn = card.querySelector('.mute-unmute-btn[data-action="mute-unmute"]');
            if (playPauseBtn) {
    ...
    playCurrentVideo() {
        ...
        if (!this.shadowRoot.contains(currentVideo)) return;
        ...
        const playPauseBtn = currentCard.querySelector('.play-pause-btn[data-action="play-pause"]');
        const muteBtn = currentCard.querySelector('.mute-unmute-btn[data-action="mute-unmute"]');
        if (playPauseBtn) {
            const playIcon = playPauseBtn.querySelector('i');
            playIcon.className = currentVideo.paused ? 'fas fa-play' : 'fas fa-pause';
        }
        if (muteBtn) {
            const muteIcon = muteBtn.querySelector('i');
            muteIcon.className = currentVideo.muted ? 'fas fa-volume-mute' : 'fas fa-volume-up';
        }
    }
    ...
    togglePlayPause(card) {
        if (!card) return;
        const currentVideo = card.querySelector('.video-element');
        const playPauseBtn = card.querySelector('.play-pause-btn[data-action="play-pause"]');
        if (!currentVideo || !playPauseBtn) {
            console.warn('Play/pause button or video element not found');
            return;
        }
            ...
            if (currentVideo.readyState >= 3) {
                ...
                const onCanPlay = () => {
                    ...
                    currentVideo.play()
                        ...
                        .catch(e => console.warn('Play after canplay failed:', e));
                };
                currentVideo.addEventListener('canplay', onCanPlay);
            }
        } else {
    ...
    toggleMute(card) {
        ...
        const currentVideo = card.querySelector('.video-element');
        const muteBtn = card.querySelector('.mute-unmute-btn[data-action="mute-unmute"]');
        if (!currentVideo || !muteBtn) {
            console.warn('Mute/unmute button or video element not found');
            return;
        }
